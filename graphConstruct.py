...
def ConHyperGraphList(WatchCounts, cascades, timestamps, user_size, step_split=Constants.step_split):
    '''split the graph to sub graphs, return the list'''

    times, root_list = ConHyperDiffsuionGraph(cascades, timestamps, user_size)
    WatchCounts, root_list = ConHyperDiffsuionGraph(cascades, WatchCounts, user_size)
    # A, D = generate_matrices(times)
    zero_vec = torch.zeros_like(times)
    one_vec = torch.ones_like(times)

    time_sorted = []
    graph_list = {}
    new_graph_list = {}  # 新增的字典来存储 new_sub_graph
    ...
    for x in range(split_length, split_length * step_split, split_length):
        ...
        # 新增代码：根据 sub_graph 生成 new_sub_graph
        new_sub_graph = torch.where(sub_graph == 1, WatchCounts, zero_vec)

        graph_list[time_sorted[x]] = sub_graph
        new_graph_list[time_sorted[x]] = new_sub_graph  # 将 new_sub_graph 添加到新字典中

    # 更改 graphs 的定义以包含 new_graph_list
    graphs = [new_graph_list, root_list]  # 将 graph_list 替换为 new_graph_list
