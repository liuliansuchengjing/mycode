...
def Split_data(data_name, train_rate=0.8, valid_rate=0.1, random_seed=300, load_dict=True, with_EOS=True):
    ...
    print("total_len:%f" % total_len)  ###
    print("len(t_cascades):%f" % (len(t_cascades)))  ###
...
def WatchCount_list(data_name, WatchCount_file, with_EOS=True):
    options = Options(data_name)
    u2idx = {}
    with open(options.u2idx_dict, 'rb') as handle:
        u2idx = pickle.load(handle)
    ...
    t_cascades = []
    WatchCounts = []
    for line in open(WatchCount_file):
        if len(line.strip()) == 0:
            continue
        WatchCountlist = []
        userlist = []
        ...
        for chunk in chunks:
            try:
                # Twitter,Douban
                if len(chunk.split()) == 2:
                    user, WatchCount = chunk.split()
                # Android,Christianity
                elif len(chunk.split()) == 3:
                    root, user, WatchCount = chunk.split()
                    if root in u2idx:
                        userlist.append(u2idx[root])
                        WatchCountlist.append(float(WatchCount))
            except:
                print(chunk)
            if user in u2idx:
                userlist.append(u2idx[user])
        ...
        if len(userlist) > 1 and len(userlist) <= 500:
            if with_EOS:
                userlist.append(Constants.EOS)
                WatchCountlist.append(Constants.EOS)
            t_cascades.append(userlist)
            WatchCounts.append(WatchCountlist)
